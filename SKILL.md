---
name: vm-recovery
description: Recover a test VM to its clean snapshot when it is in a bad state, or when SSH/run fails (e.g. after a revert changed the guest IP). Uses the `vm` CLI. Follow the numbered steps exactly.
---
...
Run every command from the project directory (where `vmconfig.json` lives).
...
2. Full reset (revert to clean snapshot + power on + wait for SSH + refresh IP):
   ...
   vm vm reset
   ```
   Expected: a success line and exit `0`. If it fails, run `vm vm doctor`,
